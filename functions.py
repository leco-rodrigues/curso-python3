...
def yes_no(question: str | None = None) -> bool:
    """
    Asks the user a yes/no question and returns the response as a boolean.

    Args:
        question (str | None, optional): Custom question to display. If None,
        a default prompt is used. Defaults to None.

    Returns:
        bool: True if the user answers 'yes' or 'y', False if 'no' or 'n'.
    """
    if question is None:
        question = "Do you want to continue? (y/n)"

    while True:
        choice: str = str(input(txt_style(f"{question}", "n") + " ")).strip().lower()
